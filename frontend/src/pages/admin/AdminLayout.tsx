...
const adminNavigation: { group: string; items: NavItem[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: "sitecontent", icon: FileText, label: "Inhalte", path: "/admin/content" },
...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? "dark bg-charcoal" : "bg-cream"}`}>
      {/* Header */}
      <header className="sticky top-0 z-50 bg-white dark:bg-charcoal border-b border-gold/10 dark:border-gold/5">
        <div className="max-w-full mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="font-display text-xl text-charcoal dark:text-white hidden sm:block">
              Vorja Admin
            </span>
          </div>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="p-2 text-slate hover:text-charcoal dark:text-slate-light dark:hover:text-white transition-colors"
            >
              {darkMode ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
            </button>
            <button
              onClick={handleLogout}
              className="flex items-center gap-2 px-3 py-2 text-slate hover:text-rose-gold transition-colors"
            >
      ...
      <div className="flex">
        {/* Sidebar */}
        <aside
          className={`fixed lg:sticky top-[65px] left-0 z-40 w-64 h-[calc(100vh-65px)] bg-white dark:bg-charcoal border-r border-gold/10 dark:border-gold/5 overflow-y-auto transition-transform ${
            ...
            {adminNavigation.map(({ group, items }) => (
              <div key={group}>
                <button
                  ...
                  className="flex items-center justify-between w-full px-3 py-2 text-xs font-semibold text-slate uppercase tracking-wider hover:text-charcoal dark:hover:text-white transition-colors"
                >
                  <span>{group}</span>
