        ...
        <tr key={i} className="border-b border-gold/5">
          {[10, 28, 16, 16, 14, 12, 14, 16].map((w, j) => (
            <td key={j} className="py-3 px-4">
              <div className={`h-4 w-${w} rounded bg-cream dark:bg-charcoal animate-pulse`} />
            </td>
          ))}
        </tr>
      ))}
...
function VariantModal({ editVariant, onClose, onSaved }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-charcoal/60 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white dark:bg-charcoal-light rounded-2xl shadow-elegant w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-gold/10">
          <div>
            ...
            {!isEdit && (
              <p className="text-sm text-slate mt-0.5">Schritt {step} von 2</p>
            )}
          </div>
          <button onClick={onClose} className="p-2 rounded-lg hover:bg-cream dark:hover:bg-charcoal text-slate">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6">
          ...
          {step === 1 && (
            <>
              <div>
                <label className="block text-sm font-medium text-charcoal dark:text-white mb-3">Box-Typ</label>
                <div className="grid grid-cols-2 gap-3">
                  {boxTypes.map((b) => (
                    ...
                    >
                      <span className="block font-medium text-charcoal dark:text-white text-sm">{b.name}</span>
                      <span className="inline-block mt-1 px-2 py-0.5 rounded-full bg-cream dark:bg-charcoal text-xs text-slate">
                        {b.box_type_display}
                      </span>
              ...
              <div>
                <label className="block text-sm font-medium text-charcoal dark:text-white mb-3">Kartenpaket</label>
                <div className="grid grid-cols-2 gap-3">
                  {cardPackages.map((p) => (
                    <button
                      key={p.id}
                    ...
                    >
                      <span className="block font-medium text-charcoal dark:text-white text-sm">{p.name}</span>
                      <span className="block text-xs text-slate mt-1">{p.card_count} Karten</span>
              ...
              {selectedBox && selectedPkg && (
                <div className="p-4 bg-cream dark:bg-charcoal rounded-xl border border-gold/20 text-sm">
                  <span className="text-slate">Basispreis: </span>
                  <span className="text-charcoal dark:text-white">€{selectedBox.base_price} + €{selectedPkg.price}</span>
              ...
              <div className="flex justify-end gap-3 pt-2">
                <button type="button" onClick={onClose} className="px-4 py-2 rounded-lg border border-gold/20 text-slate hover:bg-cream dark:hover:bg-charcoal text-sm">
                  Abbrechen
                </button>
          ...
          {step === 2 && (
            <>
              ...
              <div className="space-y-3">
                <label className="block text-sm font-medium text-charcoal dark:text-white">Preis-Konfiguration</label>
                <div className="p-3 bg-cream dark:bg-charcoal rounded-lg border border-gold/10 text-sm">
                  <span className="text-slate">Automatisch berechnet: </span>
                  <span className="font-medium text-gold">€{calcPrice()}</span>
                </div>
                <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-3">
                <label className="block text-sm font-medium text-charcoal dark:text-white">Personalisierungs-Optionen</label>
                <CheckField
                ...
                />
                <div>
                  <span className="block text-sm text-slate mb-2">Farbauswahl</span>
                  <div className="flex flex-wrap gap-2 mb-2">
                    {form.customization_options.color_choice.map((c) => (
                      <span key={c} className="flex items-center gap-1 px-2 py-1 bg-cream dark:bg-charcoal rounded-lg text-xs text-charcoal dark:text-white border border-gold/10">
                        {c}
                        <button type="button" onClick={() => removeColor(c)} className="text-slate hover:text-rose-gold">
                          <X className="w-3 h-3" />
                        </button>
                      </span>
                    ))}
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <button type="button" onClick={addColor} className="px-3 py-2 rounded-lg border border-gold/20 text-sm text-slate hover:bg-cream dark:hover:bg-charcoal">
              ...
              <div className="flex justify-between gap-3 pt-2">
                {!isEdit && (
                  <button type="button" onClick={() => setStep(1)} className="px-4 py-2 rounded-lg border border-gold/20 text-slate hover:bg-cream dark:hover:bg-charcoal text-sm flex items-center gap-1">
  ...
  return (
    <label className="flex items-center gap-2.5 cursor-pointer select-none">
      <input type="checkbox" checked={checked} onChange={(e) => onChange(e.target.checked)}
        className="w-4 h-4 rounded border-gold/30 accent-gold" />
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          ...
          <p className="text-slate mt-1 text-sm">Kombinationen aus Box-Typ, Kartenpaket und Anlass verwalten</p>
        </div>
      ...
      {/* Filter Bar */}
      <div className="bg-white dark:bg-charcoal-light rounded-xl p-4 flex flex-wrap gap-3 shadow-elegant">
        <select
          value={filters.occasion}
          onChange={(e) => setFilter("occasion", e.target.value)}
          className="px-3 py-2 border border-gold/20 rounded-lg text-sm bg-transparent text-charcoal dark:text-white focus:outline-none focus:border-gold"
        >
        ...
        <div className="relative flex-1 min-w-[180px]">
          ...
          <input
            ...
            placeholder="Suchen..."
            className="w-full pl-9 pr-3 py-2 border border-gold/20 rounded-lg text-sm bg-transparent text-charcoal dark:text-white placeholder:text-slate focus:outline-none focus:border-gold"
          />
        </div>
        ...
        {(filters.occasion || filters.box_type || filters.is_active || filters.search) && (
          <button
            ...
            className="px-3 py-2 rounded-lg border border-gold/20 text-sm text-slate hover:bg-cream dark:hover:bg-charcoal flex items-center gap-1.5"
          >
      ...
      <div className="bg-white dark:bg-charcoal-light rounded-xl overflow-hidden shadow-elegant">
          ...
          <thead className="bg-cream dark:bg-charcoal border-b border-gold/10">
            <tr>
              {["Bild", "Name", "Box-Typ", "Karten", "Anlass", "Preis", "Status", "Aktionen"].map((h) => (
                <th key={h} className="text-left py-3 px-4 text-slate font-medium text-xs uppercase tracking-wide">
                  {h}
                </th>
              ))}
            </tr>
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={8} className="py-16 text-center">
                  <Package className="w-10 h-10 text-slate-light mx-auto mb-3" />
              ...
              variants.map((v) => (
                <>
                  <tr key={v.id} className="border-b border-gold/5 hover:bg-cream/30 dark:hover:bg-charcoal/50 transition-colors">
                    <td className="py-3 px-4">
                      {v.image ? (
                        <img src={v.image} alt={v.name} className="w-10 h-10 rounded-lg object-cover border border-gold/10" />
                      ) : (
                        <div className="w-10 h-10 rounded-lg bg-cream dark:bg-charcoal border border-gold/10 flex items-center justify-center">
                          <Package className="w-4 h-4 text-slate-light" />
                        </div>
                      )}
                    </td>
                    <td className="py-3 px-4">
                      <span className="font-medium text-charcoal dark:text-white">{v.name}</span>
                      {v.is_default && (
                        <span className="ml-2 px-1.5 py-0.5 rounded text-xs bg-gold/10 text-gold">Standard</span>
                      )}
                    </td>
                    <td className="py-3 px-4 text-slate">{v.box_type_name}</td>
                    <td className="py-3 px-4 text-slate">{v.card_count}</td>
                    <td className="py-3 px-4 text-slate">{v.occasion_name ?? "—"}</td>
                    <td className="py-3 px-4 font-medium text-charcoal dark:text-white">€{v.calculated_price}</td>
                    <td className="py-3 px-4">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                        v.is_active
                          ? "bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400"
                          : "bg-slate-100 dark:bg-charcoal text-slate"
                      }`}>
                  ...
                  {confirmDeleteId === v.id && (
                    <tr key={`del-${v.id}`} className="bg-red-50 dark:bg-red-900/10 border-b border-gold/5">
                        ...
                        <div className="flex items-center justify-between gap-3 flex-wrap">
                          ...
                          <div className="flex gap-2">
                            <button
                              ...
                              className="px-3 py-1.5 rounded-lg border border-gold/20 text-sm text-slate hover:bg-cream dark:hover:bg-charcoal"
                            >
                              Abbrechen
                            </button>
                            <button
        ...
        {/* Pagination */}
        {totalCount > 0 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gold/10 bg-cream/30 dark:bg-charcoal/30">
            ...
            <div className="flex gap-2">
              <button
                ...
                disabled={page === 1}
                className="p-1.5 rounded-lg border border-gold/20 text-slate hover:bg-cream dark:hover:bg-charcoal disabled:opacity-40 disabled:cursor-not-allowed"
              >
...
function ActionBtn({
  ...
  return (
    <button
      ...
      title={title}
      onClick={onClick}
      className={`p-1.5 rounded-lg text-slate hover:text-charcoal dark:hover:text-white hover:bg-cream dark:hover:bg-charcoal transition-colors ${className}`}
    >
      {children}
    </button>
  );
